...
```{r}
plot<-data.frame(cor=c(gblup_cor,rrblup_cor),method=c(rep("GBLUP",1),rep("RRBLUP",1)))
plot
p<-data.frame(cor,method=c(rep("GBLUP",1),rep("RRBLUP",1)))
library(ggplot2)
ggplot(plot,aes(x=method,y=cor)+
       geom_boxplot(width=0.5)+
       labs(title="Prediction accuracy of GBLUP & RRBLUP",x="method",y="accuracy"))


cor<-cbind(gblup_cor,rrblup_cor)
cor
```